...
export type UiRef<Props extends GeneratorProps> = {
    ...
    controlName: string; 
    getFullName(): string;
    getInheritName(): string;
};

export function GetRef<Props extends GeneratorProps>(owningNamespace: string | undefined, controlName: string): UiRef<Props> {
...
export class UiFile extends GeneratorBase<UiFile> {
    ...
    addControl<Props extends GeneratorProps>(controlName: string, control: Control): UiRef<Props> {
...
const FRAGMENT_SYMBOL = Symbol("minecraft_fragment");

class Fragment {
    [FRAGMENT_SYMBOL] = true;
    children: Control[];

    constructor(children: Control[]) {
        this.children = children;
        this[FRAGMENT_SYMBOL] = true;
    }
}

function isFragment(x: any): x is Fragment {
    return x && x[FRAGMENT_SYMBOL] === true;
}
...
export function createMinecraftElement(
    ...
    const flattenedChildren = children
        .flat()
        .flatMap(c => isFragment(c) ? c.children : [c])
...
export function createMinecraftFragment(props: { children?: any[] }) {
